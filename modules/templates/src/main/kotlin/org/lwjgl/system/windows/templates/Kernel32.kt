...
val Kernel32 = "Kernel32".nativeClass(WINDOWS_PACKAGE, binding = simpleBinding("kernel32", callingConvention = CallingConvention.STDCALL)) {
	...
	NativeName("CreateEventW")..HANDLE(
		...
		nullable../*LPSECURITY_ATTRIBUTES*/opaque_p.IN(
			"lpEventAttributes",
			"""
		...
		nullable..LPCTSTR.IN(
			"lpName",
			"""
			the name of the event object. The name is limited to {@code MAX_PATH} characters. Name comparison is case sensitive.

			If {@code lpName} matches the name of an existing named event object, this function requests the {@code EVENT_ALL_ACCESS} access right. In this
			case, the {@code bManualReset} and {@code bInitialState} parameters are ignored because they have already been set by the creating process. If the
			...
			file-mapping object), the function fails and the #GetLastError() function returns {@code ERROR_INVALID_HANDLE}. This occurs because these objects
